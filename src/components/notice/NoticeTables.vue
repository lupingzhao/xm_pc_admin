...
import { Notices } from '../../types/index'
...
let getData = (current: number, pageSize: number, query: string) => {
  api.getNotice({ current: current, pageSize: pageSize, query: query }).
    then((res: any) => {
...
let swich = (e: string, show: boolean) => {
  api.showNotice({ id: e, isShow: show }).then((res: any) => {
