<template>
  <el-table :data="tableData" stripe border style="width: 100%">
    ...
    <el-table-column label="秒杀商品" prop="goods.name" align="center"></el-table-column>
    <el-table-column
      label="开始时间"
    ...
    <el-table-column label="秒杀价格" prop="amount" align="center">
      <template #default="scope">
        <span>￥{{ scope.row.price }}</span>
      </template>
    </el-table-column>
    <el-table-column label="秒杀数量" show-overflow-tooltip prop="goods_number" align="center"></el-table-column>
    ...
    <el-table-column label="操作" align="center" width="180">
      <template #default="scope">
        ...
        <el-popconfirm
          ...
          iconColor="red"
          title="确定将此数据删除吗？"
          @confirm="handleDelete(scope.row._id, scope.row._id)"
        >
          <template #reference>
...
import { Seckill } from '../../types/index'

// 数据
let current = ref<number>(1)
let pageSize = ref<number>(10)
let query = ref<string>('')
let tableData = ref<Seckill[]>([])
let total = ref(0)
...
let getData = (current: number, pageSize: number, query: string) => {
  api.getSeckill({ current: current, pageSize: pageSize, query: query }).
    then((res: any) => {
      res.data.map((a: Seckill) => {
...
let handleDelete = (id: string, goodsId: string) => {
  api.delSeckill({ id: id, goodsId: goodsId }).then((res: any) => {
...
let swich = (e: string, show: boolean) => {
  api.showSeckill({ id: e, isShow: show }).then((res: any) => {
